/**
 * Created by icellent on 15/12/31.
 */
var BubbleShoot = window.BubbleShoot || {};
BubbleShoot.CollisionDetector = (function ($) {
    var CollisionDetector = {
        findInterseciton: function (curBubble, board, angle) {
            var rows = board.getRows();
            var collision = null;
            var pos = curBubble.getSprite().position();
            var start = {
                left: pos.left + BubbleShoot.ui.BUBBLE_DIM / 2,
                top: pos.top + BubbleShoot.ui.BUBBLE_DIM / 2
            };
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                for (var j = 0; j < row.length; j++) {
                    ...
                    if (bubble) {
                        ...
                        if (distEC < BubbleShoot.ui.BUBBLE_DIM * .75) {
                            var dt = Math.sqrt(BubbleShoot.ui.BUBBLE_DIM * BubbleShoot.
                                    ui.BUBBLE_DIM - distEC * distEC);
                            var offset1 = {
                                x: (t - dt) * dx,
                                y: -(t - dt) * dy
                            };
